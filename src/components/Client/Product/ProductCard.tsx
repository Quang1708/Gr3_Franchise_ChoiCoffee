    ...
    <div
     ...
     className="cursor-pointer flex flex-col bg-white dark:bg-background-dark rounded-xl overflow-hidden border border-charcoal/5 dark:border-white/5 hover:shadow-xl transition-all group">
        <div className="relative aspect-4/5 w-full bg-background-light overflow-hidden">
           ...
           <div className="w-full h-full bg-center bg-no-repeat bg-cover transition-transform duration-700 group-hover:scale-110"
        ...
        <div className="p-4 flex flex-col gap-3">
          <div>
            <p className="text-clay text-[10px] font-bold uppercase tracking-wider">Mã: {item.id}</p>
            <h3 className="text-charcoal dark:text-white font-bold text-base line-clamp-2 leading-snug mt-1 min-h-12">{item.name}</h3>
          </div>
          <div className="flex items-end justify-between">
            <div className="flex flex-col">
              <p className="text-primary font-extrabold text-lg leading-none">{item.price}</p>
              {item.originalPrice ? (
                <p className="text-sm text-charcoal/50 line-through">{item.originalPrice}</p>
              ):(<p className="text-sm text-charcoal/50">-</p>)}
            </div>
            <span className="text-wood-brown text-[11px] font-medium">Kho: {item.stock} {item.unit}</span>
          </div>
          <div 
          ...
          className="flex items-center gap-2 mt-1">
            <div className="flex items-center border border-charcoal/10 dark:border-white/10 rounded-lg overflow-hidden h-10 bg-background-light/50 dark:bg-white/5">
              <button className="px-2.5 text-charcoal dark:text-white hover:bg-charcoal/5">-</button>
              <input 
              title="quantity"
              className="w-8 text-center border-none bg-transparent p-0 text-sm font-bold text-charcoal dark:text-white focus:ring-0" type="text" value="1"/>
              <button className="px-2.5 text-charcoal dark:text-white hover:bg-charcoal/5">+</button>
            </div>
            <button 
              title="add-to-cart"
              className="cursor-pointer flex-1 bg-charcoal dark:bg-white text-white dark:text-charcoal h-10 rounded-lg text-xs font-bold hover:bg-primary dark:hover:bg-primary dark:hover:text-white transition-all flex items-center justify-center gap-1"
