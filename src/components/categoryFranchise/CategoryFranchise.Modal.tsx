...
const CategoryFranchiseCreateModal = ( { isOpen, onClose, onSuccess, franchiseId, showFranchiseSelect }: categoryFranchiseCreateModalProps ) => {
                ...
                <ul className="absolute z-50 w-full mt-2 py-1 overflow-auto bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 focus:outline-none">
                  {filteredFranchises.length > 0 ? (
                    ...
                    <li className="px-4 py-6 text-center">
                      <span className="text-sm text-gray-400">Không tìm thấy chi nhánh</span>
                    </li>
                  )}
                </ul>
              )}
                ...
                <ul className="custom-scroll absolute z-50 w-full mt-2 py-1 overflow-auto bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 focus:outline-none">
                  {filteredCategories.length > 0 ? (
                    filteredCategories.map((category) => {
                      const isExisting = existingCategoryIds.has(category.value);
                      return (
                      <li
                        key={category.code}
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {isExisting ? (
                            <span className=" text-primary" aria-label="already-exists">
                              <Check  />
                            </span>
                          ) : null}
                        </div>
