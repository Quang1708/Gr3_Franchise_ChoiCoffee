...
const FranchiseSelect = ({ isOpen, franchises, selectedFranchise, onSelectFranchise }: Props) => {
  const selectedBranchId = localStorage.getItem("selectedFranchise");
    const selectedBranch = franchises.find(franchise => franchise.id === selectedBranchId);
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 py-2 max-h-80 overflow-y-auto z-50 animate-in fade-in slide-in-from-top-2 hide-scrollbar">
