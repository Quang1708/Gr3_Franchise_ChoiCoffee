...
const ProductFranchisePage = () => {
    ...
    const [currentFilters, setCurrentFilters] = useState<any>({ is_active: "", is_deleted: "false" });
    ...
    const [selectedItem, setSelectedItem] = useState<ProductFranchise | null>(null);
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                res.data.forEach((item: any) => {
                    if (item.franchise_id) fMap.set(String(item.franchise_id), item.franchise_name);
                    if (item.product_id) pMap.set(String(item.product_id), item.product_name);
                    if (item.size) sMap.set(String(item.size), `Size ${item.size}`);
                });
                setFranchiseOptions(Array.from(fMap, ([value, label]) => ({ value, label })));
                setProductOptions(Array.from(pMap, ([value, label]) => ({ value, label })));
                setSize(Array.from(sMap, ([value, label]) => ({ value, label }))
    ...
    const handleStatusChange = async (item: ProductFranchise, newStatus: boolean) => {
        try {
            setIsLoading(true);
            const res = await changeStatusProductFranchiseUsecase(item.id, newStatus);
            if (res) {
                setProductList(prev => prev.map(p => p.id === item.id ? { ...p, is_active: newStatus } : p));
    ...
    const handleConfirmAction = async () => {
        ...
        try {
            ...
            fetchProducts(page, "full");
            toast.success(type === "delete" ? "Đã chuyển vào thùng rác" : "Khôi phục thành công");
        } catch { toast.error("Thao tác thất bại"); }
    };
    ...
    const columns: Column<ProductFranchise>[] = useMemo(() => [
        ...
        { header: "Sản phẩm", accessor: "product_name", className: "font-semibold text-blue-600" },
        { header: "Size", accessor: "size" },
        {
            header: "Giá cơ bản",
            accessor: "price_base",
            render: (item) => <span className="font-bold text-green-600">{item.price_base?.toLocaleString()}đ</span>
        },
    ], [isAdmin]);
    ...
    return (
        <>
            ...
            <CRUDPageTemplate<ProductFranchise>
                ...
                onPageSizeChange={(s) => { setPageSize(s); fetchProducts(1, "full", s); }}
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full">
                        {isAdmin && (
                            <div className="flex-1">
                                <FormSelect
                                    label=""
                                    options={[{ label: "Tất cả chi nhánh", value: "" }, ...franchiseOptions]}
                ...
                onAdd={() => { setFormMode("create"); setSelectedItem(null); setIsFormOpen(true); }}
                onEdit={(item) => { setFormMode("edit"); setSelectedItem(item); setIsFormOpen(true); }}
                onView={(item) => { setFormMode("view"); setSelectedItem(item); setIsFormOpen(true); }}
                ...
                filters={[
                    ...
                    { key: "is_active", label: "trạng thái", options: [{ value: "true", label: "Hoạt động" }, { value: "false", label: "Ngưng hoạt động" }] },
            ...
            <ActionConfirmModal
                ...
                title={modalConfig.type === "delete" ? "Xác nhận xóa" : "Xác nhận khôi phục"}
