...
const ShiftSidebarSection = ({
  ...
  return (
    <aside className="rounded-2xl border border-gray-200 bg-white p-4 xl:col-span-4 xl:flex xl:min-h-0 xl:flex-col xl:overflow-hidden">
      ...
      <div
        className={`${hideCreateSection ? "mt-0" : "mt-5"} border-t border-gray-100 pt-4 xl:min-h-0 xl:flex-1 xl:overflow-hidden`}
      >
        ...
        {!canShiftRead && !canShiftWrite ? (
          ...
          <div className="max-h-80 space-y-2 overflow-auto pr-1 xl:max-h-none xl:h-full scrollbar-hide">
            {visibleShifts.map((item) => (
              ...
              >
                ...
                {showFranchiseLabel && item.franchise_id && (
                  <div className="text-xs text-gray-500">
                    {item.franchise_name ||
                      franchiseNameMap[item.franchise_id] ||
                      item.franchise_id}
                  </div>
                )}
              </div>
