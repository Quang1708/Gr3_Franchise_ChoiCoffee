...
export const UserForm = ({
    ...
    initialData?: User;
    onSubmit: (data: UserFormValues, setError: (field: keyof UserFormValues, error: { message: string }) => void) => void;
    isOpen: boolean;
    isLoading?: boolean;
    onClose: () => void;
                    ...
                    <div>
                        ...
                        {isView ? (
                            <div className="py-2 min-h-[38px] border-b border-gray-100 md:border-none">
                                <span className="text-sm text-gray-900">{initialData?.roleCode || "—"}</span>
                            </div>
                        ) : (
                            <select
                                id="roleCode"
                        ...
                        {errors.roleCode && (
                            <span className="text-xs text-red-500 mt-1 block">{errors.roleCode.message}</span>
                        )}
                    </div>
                    {mode === "create" && (
                        <FormInput
