          ...
          <div className="w-full lg:w-2/3 space-y-4">
            <div className="flex flex-col border border-slate-100 rounded-lg shadow-sm bg-white overflow-hidden">
              ...
              <div className="divide-y divide-slate-50">
                {cartItems.map((item) => (
                  <div key={item.product_franchise_id} className="p-4 grid grid-cols-1 md:grid-cols-12 items-center gap-4 hover:bg-slate-50/50 transition-colors">
                    <div className="col-span-5 flex gap-4">
                      <input type="checkbox" defaultChecked className="mt-4 rounded border-slate-300 text-primary" />
                      <div className="size-20 rounded-md overflow-hidden border border-slate-100 shrink-0 bg-slate-50">
                        <img src={item.image_url} alt={item.product_name} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex flex-col justify-center">
                        <h3 className="font-bold text-slate-800 line-clamp-1">{item.product_name}</h3>
                        <p className="text-sm text-slate-400 mt-0.5 italic">SKU: {item.SKU}</p>
                    ...
                    <div className="col-span-2">
                      <div className="flex items-center justify-center">
                        <button className="w-8 h-8 flex items-center justify-center border border-slate-200 rounded-l hover:bg-slate-100">-</button>
                        <input type="text" value={item.quantity} className="w-10 h-8 border-y border-x-0 border-slate-200 text-center focus:ring-0" readOnly />
                        <button className="w-8 h-8 flex items-center justify-center border border-slate-200 rounded-r hover:bg-slate-100 text-primary font-bold">+</button>
                      </div>
                    </div>
                    ...
                    <div className="col-span-1 text-right">
                      <button className="text-slate-400 hover:text-red-500 transition-colors p-2 rounded-full hover:bg-red-50">
                        <span className="material-symbols-outlined text-xl font-light">delete</span>
                      </button>
                    </div>
                  </div>
                ))}
            ...
            <div className="bg-white border border-slate-100 rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className="flex justify-between">
                  <div className="flex items-center gap-1.5 text-slate-500">
                    ...
                    <span className="bg-orange-100 text-primary text-[10px] px-1.5 font-bold rounded">PLATINUM</span>
                  </div>
                ...
                <div className="pt-4 border-t border-slate-100 flex flex-col gap-2">
                  <div className="flex justify-between items-end">
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-black text-primary leading-none">₫2.565.000</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 flex flex-col gap-3 border-t border-slate-50">
                <div className="flex items-start gap-3 text-xs text-slate-500 italic leading-relaxed">
                  ...
                  Dữ liệu nhập hàng sẽ được lưu vào hệ thống Audit Log để quản lý đối soát.
                </div>
              </div>
            </div>
          </div>
